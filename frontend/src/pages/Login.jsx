  ...
  useEffect(() => {
        ...
        callback: async (response) => {
          if (!response?.credential) {
            setModal({
              ...
              message: "Google did not return a valid token.",
            });
            return;
          }
          ...
          try {
            ...
            if (payload?.success) {
              ...
              setModal({
                ...
                message:
                  payload.message ||
                  "Your Google account has been connected. Press OK to continue.",
              });
            } else {
              setModal({
                open: true,
    ...
    const loadGoogleSignIn = async () => {
      try {
        ...
        const clientId =
          response.data?.data?.trim?.() || response.data?.data || "";
        ...
        if (existingScript) {
          existingScript.addEventListener("load", startGoogle, { once: true });
          setGoogleConfigLoading(false);
          return () => {
            existingScript.removeEventListener("load", startGoogle);
          };
        }
        ...
        script.onerror = () => {
          if (!cancelled) {
            ...
            setModal({
              ...
              message:
                "Unable to load Google sign-in. Check your internet connection and backend configuration.",
            });
          }
        };
        ...
        <div className="w-full rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl">
          ...
          <div className="mt-5 rounded-2xl border border-slate-700 bg-slate-950/70 p-4">
            <div className="flex flex-col items-center gap-3">
              <div className="w-full min-h-[48px] rounded-full border border-slate-600 bg-white px-2 py-2 sm:w-auto sm:min-w-[320px]">
                <div ref={googleButtonRef} className="flex justify-center" />
              </div>
              {googleReady && (
                <p className="text-center text-xs text-slate-400">
                  Click the Google button above to open the OAuth sign-in flow.
                </p>
              )}
              {!googleReady && !googleConfigLoading && (
                <p className="text-center text-xs text-slate-400">
                  Google sign-in is disabled until the backend Google client ID
                  is configured.
                </p>
              )}
              {googleConfigLoading && (
